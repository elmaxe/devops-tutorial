...
This code implements a function `review()`, which “reviews” the year you pass to it, returning a string. It’s time to write some unit tests for this function. Create the file `test_app.py` in the `test`-folder and paste the following code:
```python
import unittest
...
This file can sure look a bit complicated if you are not used to unit tests. First and foremost we import the `unittest` library so that we can use its functions. After that we create a class that extends the `unittest.TestCase` class. All unit test functions in this class must begin with `test` in order for unittest to recognize them as unit tests. The class contains four different unit tests that test the review function in different ways.

There are many different ways to design your unit tests. We have used three different kinds of unit tests: `assertEqual`, `assertRaises` and `assertIn`, but there are many more you can use. If you are interested, read the [unittest documentation](https://docs.python.org/3/library/unittest.html).
...
# Run the code
Run `python3 -m unittest` to run the unit tests. If everything is set up correctly, it should tell you that it ran some number of tests and if they succeeded or not.
...
```
to the job specification. Then we can modify the “Set Up Python” step to genetically handle the different versions:
```yml
- name: Set up Python ${{ matrix.python-version }}
      uses: actions/setup-python@v2
      with:
...
### Run tests
The tests are run with `python -m unittest`
```yml
    - name: Test
      run: python -m unittest
```
